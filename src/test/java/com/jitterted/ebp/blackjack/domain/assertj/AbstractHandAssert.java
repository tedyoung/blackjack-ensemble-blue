...
public abstract class AbstractHandAssert<S extends AbstractHandAssert<S, A>, A extends Hand> extends AbstractObjectAssert<S, A> {
    ...
    static boolean isAnyCardFaceDown(List<Card> cards) {
        return cards.stream()
                    .anyMatch(card -> card.isFaceDown());
    }

    public void allCardsFaceUp() {
        boolean anyCardFaceDown = isAnyCardFaceDown(actual.cards());
        if (anyCardFaceDown) {
            String assertjErrorMessage = "\nExpecting hand:\n <%s>\nto have all the cards face up, but at least one was face down :\n  <%s>";
            failWithMessage(assertjErrorMessage, actual, actual.cards());
        }
    }
}
